defmodule ApiWeb.AccountDetailsControllerTest do
  ...
  setup %{conn: conn} do
    ...
    [
      conn: conn,
      token: token,
      index: Routes.account_details_path(conn, :index, account_id),
      not_found: Routes.account_details_path(conn, :index, invalid_account_id)
    ]
  end

  describe "GET /accounts/:account_id/details" do
    ...
    test "should return account details", %{
      ...
      account_details = json_response(http_response, 200)

      links = %{
        "account" => "#{@endpoint.url()}/accounts/8b6d213d-1d1a-5caa-a2d9-ff3d28117c6f",
        "self" => "#{@endpoint.url()}/accounts/8b6d213d-1d1a-5caa-a2d9-ff3d28117c6f/details"
      }

      assert %{
               "account_id" => "8b6d213d-1d1a-5caa-a2d9-ff3d28117c6f",
               "account_number" => 891_824_333_886,
               "links" => ^links,
               "routing_numbers" => %{"ach" => "581553886"}
             } = account_details
    end
  end
end
